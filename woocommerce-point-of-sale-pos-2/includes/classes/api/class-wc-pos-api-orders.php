	...
	public function edit_order( $id, $data ) {
		...
		try {
			...
			if( $data['create_account'] === true ) {
				...
				update_post_meta( $order->id, '_customer_user', $new_customer );
			}
			...
			$items = $wpdb->get_results( $wpdb->prepare( "SELECT order_item_id FROM {$wpdb->prefix}woocommerce_order_items WHERE order_id = %d", $order->id ) );
			if( $items ){
